...
static const char* get_number(struct parsed_symbol* sym)
{
    ...
    else if (*sym->current >= 'A' && *sym->current <= 'P')
    {
        ...
        sprintf_s(ptr, sizeof(void*) + 17, "%s%d", sgn ? "-" : "", ret);
        sym->current++;
    }
    else return NULL;
    return ptr;
...
static BOOL handle_method(struct parsed_symbol* sym, BOOL cast_op)
{
    ...
    mark = sym->stack.num;
    args_str = get_args(sym, &array_pmt, TRUE, '(', ')');
    if (!args_str) goto done;
    ...
    void* unknown, unsigned short int flags)
{
    ...
    if (buffer && buflen)
    {
        strncpy_s(buffer, buflen, result, buflen);
    }
    else
    {
        size_t len = strlen(result) + 1;
        buffer = malloc(len);
        if (buffer) strcpy_s(buffer, len, result);
    }
